...
if __name__ == "__main__":
    ...
    # codes with preprocessed data in "Outputs" is provided in backup project, you can replicate directly to here.
    # or run the backup project (almost the same with this one just with dataset provided).
    # if you don't use backup project:
    # notice that two files needed to download for the project, one is .npz file (with .csv),
    # the other is the image datasets with .png format, which will all be downloaded by the program automatically.
    # my suggestion is to use my backup project directly with preprocessed dataset provided.
