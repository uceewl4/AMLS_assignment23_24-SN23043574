...
class EnsembleNet(Model):
    ...
    def __init__(self, lr):
        ...
        self.lr = lr
        self.w1_model = MLP(task="A", method="MLP", lr=lr)  # model component MLP
        self.w2_model = CNN(task="A", method="CNN", lr=lr)  # model component CNN
    ...
    def weight_selection(self, train_ds, val_ds):
        ...
        for ratio in ratios:
            for val_images, val_labels in val_ds:
                ...
                total_predictions = (
                    ratio * w1_predictions + (1 - ratio) * w2_predictions
                )
                val_prob = tf.nn.sigmoid(total_predictions)  # probability
                for i in val_prob:
                    val_pred.append(1) if i >= 0.5 else val_pred.append(
                        0
                    )  # predicted label
        ...
        for train_images, train_labels in train_ds:
            ...
            total_predictions = (
                self.weight * w1_predictions + (1 - self.weight) * w2_predictions
            )
  ...
  """
    ...
    def test(self, test_ds):
        ...
        for test_images, test_labels in test_ds:
            ...
            test_prob = tf.nn.sigmoid(total_predictions)  # probability
