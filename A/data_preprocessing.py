...
from medmnist import (
    PneumoniaMNIST,
)  # cannot be used on gpu but can be used on my cpu, maybe because of package version
...
def zoom(img):
    h, w, c = img.shape
    imgZoom = cv2.resize(
        img, (int(0.5 * w), int(0.5 * h)), interpolation=cv2.INTER_CUBIC
    )
...
def data_augmentation(path, length_1, length_0, pre_file_0, mode=None):
    ...
    flip_index = random.sample(
        [i for i in range(length_0)], int((length_1 - length_0) / 5)
    )
    for i in flip_index:
    ...
    rot_index = random.sample(
        [i for i in range(length_0)], int((length_1 - length_0 - len(imgs)))
    )
    for i in rot_index:
        img = cv2.imread(os.path.join(path, f"{pre_file_0[i]}"))
        imgRot = rotation(img)
...
param {*} download: whether download dataset, if you don't use provided backup project and want to check the project
        from original dataset, please use the argument here. Detailed guideline is shown in README.md and Github link.
return {*}: npz data
"""


def load_data_log4A(download=False):
    if download == True:
        download_dataset = PneumoniaMNIST(
            split="train", download=True, root="Datasets/"
        )
        os.system(
            "python -m medmnist save  --flag=pneumoniamnist --postfix=png --folder=Datasets/ --root=Datasets/"
        )
    ...
    print(
        f"Train data length: {len(data['train_images'])}, label 0: {np.count_nonzero(data['train_labels'].flatten() == 0)}, label 1: {np.count_nonzero(data['train_labels'].flatten() == 1)}"
    )
    print(
        f"Validation data length: {len(data['val_images'])}, label 0: {np.count_nonzero(data['val_labels'].flatten() == 0)}, label 1: {np.count_nonzero(data['val_labels'].flatten() == 1)}"
    )
    print(
        f"Test data length: {len(data['test_images'])}, label 0: {np.count_nonzero(data['test_labels'].flatten() == 0)}, label 1: {np.count_nonzero(data['test_labels'].flatten() == 1)}"
    )
...
def data_preprocess4A(raw_path):
    ...
    data = load_data_log4A(download=True)
    ...
    for index, f in enumerate(raw_file):
        if not os.path.isfile(os.path.join(raw_path, f)):
            ...
            cv2.imwrite(
                os.path.join("Outputs/pneumoniamnist/preprocessed_data", f"{f}"),
                imgGamma,
            )
    ...
    for index, f in enumerate(
        pre_file
    ):  # construct individual train/validation/test files aggregation
    ...
    new_train_length = data_augmentation(
        pre_path,
        len(data["train_images"]) - len(pre_file_train_0),
        len(pre_file_train_0),
        pre_file_train_0,
        mode="train",
    )
    new_test_length = data_augmentation(
        pre_path,
        len(data["test_images"]) - len(pre_file_test_0),
        len(pre_file_test_0),
        pre_file_test_0,
        mode="test",
    )
    new_val_length = data_augmentation(
        pre_path,
        len(data["val_images"]) - len(pre_file_val_0),
        len(pre_file_val_0),
        pre_file_val_0,
        mode="val",
    )
