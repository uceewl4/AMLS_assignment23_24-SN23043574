...
## Usage
- Step 1: Create environment with corresponding version mentioned in **Requirements**.
...
**For running the backup project:**
- Step 3:
  ...
  <font color=#008000>There are several arguments which can be specified to select different model scenario. Default scenario is LR baseline for task A. You can specify the argument as follows:</font>
    ...
    --pre_data: whether perform data preprocessing, default False. <font color=red>only used for the other project where you don't have any data and want to check the process of data preprocessing.</font>
  ...
  You can also try some small epochs or see my video to check implementation.
  </font>
...
- Step 3: There are two datastes for each task you need to download or replicate from backup project. The first one is through command lines in terminal:
  <p><code>python -m medmnist save --flag=pneumoniamnist --postfix=png --folder=Datasets/ --root=Datasets/</code></p>
  <p><code>python -m medmnist save --flag=pathmnist --postfix=png --folder=Datasets/ --root=Datasets/</code></p>
    The second one is downloaded by the project with argument configuration of "--npz True".

- Step 4:
  <p><code>python main.py --npz True --pre_data True</code></p>
  <font color=#008000>Notice that these two arguments of npz and pre_data must be set as True to check data preprocessing. There are several other arguments which can be specified to select different model scenario. Default scenario is LR baseline for task A. You can specify the argument as follows:</font>
    ...
    --pre_data: whether perform data preprocessing, default False. <font color=red>must be set as True for this project where you don't have any data and want to check the process of data preprocessing, no need for backup project</font>

    --npz: whether download npz data, default False. <font color=red>must be set as True for this project where you don't have any data and want to check the process of data preprocessing, no need for backup project</font>
...
- To run DDI system:
  ...
  <font color=red>The system can only be run with all dataset and preprocessed one already prepared. You cannot run it with only codes provided. It's suggested to try it in backup project with model implementation together.</font>
